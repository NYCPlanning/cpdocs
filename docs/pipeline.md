...
| :-- | :-- | :-- | :-- |
|1 | DOB Milestones | DOB permit database, shared periodically w/ DCP
|2 | Job Status  | Most recent permit status, listed in DOB permit data (see Data Dictionary for futher detail)
|3 | Proposed / Existing Occupancy Codes | Occupancy types listed in initial permit application (e.g., single vs. multi-family dwelling)
|4 | Job Number  | Unique identifier generated by DOB for all permits 
|5 | Proposed / Existing Dwelling Units | Dwelling units listed in initial permit application
|6 | Job Description [TBD] | Description of work listed in initial permit application
|7 | COs and TCOs issued (2010-2016)  | DOB dataset on CofOs issued between 1/1/2010 through 9/30/2016, provided to DCP as needed
|8 | CofO Type  | Distinguishes Final CofOs versus Temporary CofOs 
|9 | NHMP 2010-2013 and HNY 2014-2016 | HPD project datasets: 2010-13 data provided to DCP; data 2014-onward to be published by HPD via Open Data quarterly
|10 | Permit incremental units  | Calculated increment between existing units and proposed units listed in permit
|11 | CofO units | Number of units listed in most recent DOB CofO
|12 | CofO incremental units  | Calculated increment between existing units (listed in permit) and completed units (listed in CofOs)
|13 | Partial complete | Status assigned by DCP when most recent CofO lists <80% of proposed units in permit
|14 | Complete | Status assigned by DCP when most recent CofO lists >80% of proposed units in permit
|15 | Complete incremental units | Field created by DCP to capture completed incremental units (i.e. CofO incremental units w/ adjustments as needed) 
||16 | Permit outstanding | Status assigned by DCP when first permit has been issued but no CofOs have been issued
|17 | Permit pending | Status assigned by DCP when permit application is complete but first permit has not been issued
|18 | Outstanding incremental units | Field created by DCP to capture incremental units where permit issued but no CofO issued OR CofO lists <80% of permit units
|19 | Permit pending incremental units | Field created by DCP to capture incremental units where permit application is complete but first permit has not been issued
|20 | HPD total units | Total number of units listed in HPD project data, affordable and market rate (see Data Dictionary for further detail)
|21 | DCP Pipeline Category   | Field created by DCP to capture record type (Residential new; Residential renovation; Hotel new; Hotel renovation) 
|22 | DCP Pipeline Status | Field created by DCP to capture record statuses (Complete, Partial complete, Permit outstanding, Permit pending)
|23 | DCP Pipeline Units  | Field created by DCP to determine point size for mapping
- If status is Complete, set to Complete incremental units
- Otherwise, set to HPD units or Permit incremental units (if HPD n/a)
